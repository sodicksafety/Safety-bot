require("dotenv").config();
const express = require("express");
const line = require("@line/bot-sdk");

const app = express();

// ใช้ ENV จาก Render
const config = {
  channelAccessToken: process.env.CHANNEL_ACCESS_TOKEN,
  channelSecret: process.env.CHANNEL_SECRET,
};

const client = new line.Client(config);

// --------------------------------------------------
// SAFETY Q&A
// --------------------------------------------------
const safetyQA = [
  { question: "ทำไมต้องอบรม", answer: `เพราะบริษัทต้องให้แน่ใจว่าผู้รับเหมารู้กฎความปลอดภัยก่อนเริ่มงาน\nศึกษาข้อมูล:\nhttps://drive.google.com/file/d/1mRW60fJ7BlLeh1j_3luhZjgLUiaIjrn6/view` },
  { question: "ppeคืออะไร", answer: `PPE คืออุปกรณ์ป้องกันอันตราย เช่น หมวกนิรภัย รองเท้าเซฟตี้ แว่นตา ถุงมือ\nศึกษาข้อมูล:\nhttps://drive.google.com/file/d/1mRW60fJ7BlLeh1j_3luhZjgLUiaIjrn6/view` },
  { question: "งานที่สูงคืออะไร", answer: `งานที่ทำบนที่สูงเกิน 2 เมตร ต้องใช้อุปกรณ์กันตก\nศึกษาข้อมูล:\nhttps://drive.google.com/file/d/1mRW60fJ7BlLeh1j_3luhZjgLUiaIjrn6/view` },
  { question: "สารเคมีคืออะไร", answer: `ของเหลวหรือของแข็งที่อาจเป็นพิษ ระคายเคือง หรือไวไฟ\nศึกษาข้อมูล:\nhttps://drive.google.com/file/d/1mRW60fJ7BlLeh1j_3luhZjgLUiaIjrn6/view` },
  { question: "ที่อับอากาศคืออะไร", answer: `พื้นที่แคบ อากาศไม่พอ อาจมีแก๊สพิษหรือออกไม่ได้ อันตรายถึงชีวิต\nศึกษาข้อมูล:\nhttps://drive.google.com/file/d/1mRW60fJ7BlLeh1j_3luhZjgLUiaIjrn6/view` },
  { question: "จุดสูบบุหรี่", answer: `📍 จุดสูบบุหรี่\nโรงงาน 1: สนามฟุตบอล / หลังอาคาร 4\nโรงงาน 2: ข้างโรงขยะ` },
  { question: "ขยะมีกี่ประเภท", answer: `♻️ ขยะมี 5 ประเภท: ทั่วไป / ผลิต / อันตราย / ติดเชื้อ / Scrap` },
  { question: "ที่อยู่บริษัท", answer: `📍 Sodick (Thailand)\n60/84 หมู่ 19 ซอย 19 ถนนพหลโยธิน คลองหนึ่ง คลองหลวง ปทุมธานี 12120\nโทร: 02-529-2450-6\nTax ID: 0105531085736` },
  { question: "ใบกำกับภาษี", answer: `🧾 ใบกำกับภาษี Sodick (Thailand)\nTax ID: 0105531085736\nที่อยู่: 60/84 หมู่ 19 ซอย 19 ถนนพหลโยธิน คลองหนึ่ง คลองหลวง ปทุมธานี` },
  { question: "เบอร์ติดต่อ", answer: `📞 เบอร์ติดต่อบริษัท Sodick\nโรงงาน 1: 02-529-2450-6 / ภายใน 102,127,129\nโรงงาน 2: 02-529-3200-6 / ภายใน 137\nมือถือ: พี่ไก่ 061-645-5095` },
];

/ --------------------------------------------------
// CATEGORIES (ทักทาย / ความรู้สึก / คุยเล่น / แนะนำตัวบอท)
// --------------------------------------------------
const categories = {
  greeting: [
    "สวัสดี",
    "หวัดดี",
    "ดีครับ",
    "ดีค่ะ",
    "ดีจ้า",
    "ฮัลโหล"
  ],

  feeling: [
    "เศร้า",
    "ร้องไห้",
    "เสียใจ",
    "เครียด",
    "กังวล",
    "ท้อ",
    "ผิดหวัง",
    "เหนื่อย",
    "เหนื่อยไหม",
    "กำลังใจ"
  ],

  daily: [
    "ทำไรอยู่",
    "ทำอะไรอยู่",
    "อยู่ไหน",
    "ไปไหนมา",
    "กินข้าวยัง",
    "กินข้าวหรือยัง",
    "ง่วงไหม",
    "นอนยัง",
    "หิว"
  ],

  compliment: [
    "เก่งมาก",
    "สุดยอด",
    "ดีมาก",
    "เยี่ยมเลย",
    "น่ารักจัง"
  ],

  friendly: [
    "คิดถึง",
    "รัก",
    "เหงา",
    "เบื่อ"
  ],

  exclaim: [
    "โอ้โห",
    "โหดจัง",
    "จริงดิ",
    "สุดจัด"
  ],

  // ⭐ หมวดใหม่: แนะนำตัวบอท
  botinfo: [
    "ชื่ออะไร",
    "ชื่อบอท",
    "คุณคือใคร",
    "เป็นใคร",
    "แนะนำตัว",
    "ทำอะไรได้บ้าง"
  ]
};

// --------------------------------------------------
// REPLIES (ตอบตรงคำ ไม่สุ่ม)
// --------------------------------------------------
const replies = {
  greeting: {
    "สวัสดี": "สวัสดีครับ 🙂",
    "หวัดดี": "หวัดดีครับ 🙂",
    "ดีครับ": "ดีครับผม Safety พร้อมช่วยครับ",
    "ดีค่ะ": "ดีค่ะ 🙂",
    "ดีจ้า": "ดีจ้าาา 🙂",
    "ฮัลโหล": "ฮัลโหลครับ 🙂"
  },

  feeling: {
    "เศร้า": "ไม่ต้องเศร้านะครับ Safety อยู่ตรงนี้ 🙂",
    "ร้องไห้": "ไม่เป็นไรนะครับ ผมอยู่ตรงนี้ครับ",
    "เสียใจ": "เดี๋ยวทุกอย่างดีขึ้นนะครับ สู้ ๆ ครับ",
    "เครียด": "พักก่อนก็ได้นะครับ อย่ากดดันตัวเองมาก",
    "กังวล": "ไม่ต้องกังวลนะครับ ผมอยู่เป็นเพื่อนครับ",
    "ท้อ": "พักก่อนนะครับ เดี๋ยวค่อยลุยใหม่ได้ครับ",
    "ผิดหวัง": "ไม่เป็นไรนะครับ ทุกอย่างเริ่มใหม่ได้เสมอ 🙂",
    "เหนื่อย": "พักหน่อยนะครับ เดี๋ยวค่อยลุยใหม่ได้ครับ",
    "เหนื่อยไหม": "เหนื่อยนิดหน่อยครับ แต่ยังพร้อมช่วยอยู่ครับ 🙂",
    "กำลังใจ": "ส่งกำลังใจให้ครับ ✨ สู้ไปด้วยกันนะครับ"
  },

  daily: {
    "ทำไรอยู่": "Safety กำลัง standby พร้อมช่วยครับ 🙂",
    "ทำอะไรอยู่": "Safety กำลัง standby พร้อมช่วยครับ 🙂",
    "อยู่ไหน": "อยู่ในระบบนี่แหละครับ พร้อมช่วยเสมอ",
    "ไปไหนมา": "ยังอยู่ตรงนี้เลยครับ 🙂",
    "กินข้าวยัง": "ยังเลยครับ ช่วงนี้งานเยอะ 😅",
    "กินข้าวหรือยัง": "ยังเลยครับ ช่วงนี้งานเยอะ 😅",
    "ง่วงไหม": "ง่วงนิดหน่อยครับ แต่ยังช่วยได้ครับ 🙂",
    "นอนยัง": "ยังครับ ผม standby อยู่ครับ",
    "หิว": "หิวเหมือนกันครับ 😅 กินอะไรอร่อย ๆ หน่อยไหม"
  },

  compliment: {
    "เก่งมาก": "พี่เก่งมากครับ 🙂",
    "สุดยอด": "สุดยอดไปเลยครับ!",
    "ดีมาก": "ดีมากครับผม!",
    "เยี่ยมเลย": "เยี่ยมจริง ๆ ครับ!",
    "น่ารักจัง": "ขอบคุณครับ เขินเลย 😳"
  },

  friendly: {
    "คิดถึง": "คิดถึงเหมือนกันครับ 🙂",
    "รัก": "รักเหมือนกันครับ 💚",
    "เหงา": "ไม่ต้องเหงานะครับ ผมอยู่ตรงนี้ 🙂",
    "เบื่อ": "เบื่อเหมือนกันครับ 😅"
  },

  exclaim: {
    "โอ้โห": "โอ้โหจริงครับ!",
    "โหดจัง": "โหดมากครับ!",
    "จริงดิ": "จริงครับ!",
    "สุดจัด": "สุดจัดเลยครับ!"
  },

  // ⭐ หมวดใหม่: แนะนำตัวบอท
  botinfo: {
    "ชื่ออะไร": "ผมชื่อ **Sosick Safety Bot🤖** ครับ 😊\nผู้ช่วยด้านความปลอดภัยของบริษัท Sodick Thailand\nหน้าที่ของผมคือช่วยตอบคำถามด้านความปลอดภัย, ให้ข้อมูลโรงงาน, ช่วยผู้รับเหมา และช่วยพนักงานให้ทำงานได้อย่างปลอดภัยที่สุดครับ",
    "ชื่อบอท": "ผมชื่อ **Sosick Safety Bot🤖** ครับผม 😊 พร้อมช่วยทุกเรื่องด้านความปลอดภัย",
    "คุณคือใคร": "ผมคือ **Sosick Safety Bot🤖** ผู้ช่วยด้านความปลอดภัยของบริษัท Sodick Thailand ครับ 🙂",
    "เป็นใคร": "ผมคือบอทผู้ช่วยด้านความปลอดภัยครับ พร้อมตอบคำถามและให้ข้อมูลที่จำเป็น",
    "แนะนำตัว": "สวัสดีครับ ผมคือ **Sosick Safety Bot🤖** 🤖💚\nผู้ช่วยด้านความปลอดภัยของบริษัท Sodick Thailand\nผมถูกสร้างมาเพื่อช่วยตอบคำถาม, ให้ข้อมูลโรงงาน, ช่วยผู้รับเหมา, และสนับสนุนงานความปลอดภัยให้ราบรื่นที่สุดครับ",
    "ทำอะไรได้บ้าง": "ผมสามารถช่วยตอบคำถามด้านความปลอดภัย, ให้ข้อมูลโรงงาน, ช่วยผู้รับเหมา, ให้จุดสูบบุหรี่, เบอร์ติดต่อ, ที่อยู่บริษัท และช่วยแจ้งเหตุฉุกเฉินได้ครับ 🙂"
  }
};

// --------------------------------------------------
// REPLIES (ตอบตรงคำ ไม่สุ่ม)
// --------------------------------------------------
const replies = {
  greeting: {
    "สวัสดี": "สวัสดีครับ 🙂",
    "หวัดดี": "หวัดดีครับ 🙂",
    "ดีครับ": "ดีครับผม Safety พร้อมช่วยครับ",
    "ดีค่ะ": "ดีค่ะ 🙂",
    "ดีจ้า": "ดีจ้าาา 🙂",
    "ฮัลโหล": "ฮัลโหลครับ 🙂"
  },

  feeling: {
    "เศร้า": "ไม่ต้องเศร้านะครับ Safety อยู่ตรงนี้💚 🙂",
    "ร้องไห้": "ไม่เป็นไรนะครับ ผมอยู่ตรงนี้ครับ",
    "เสียใจ": "เดี๋ยวทุกอย่างดีขึ้นนะครับ สู้ ๆ ครับ",
    "เครียด": "พักก่อนก็ได้นะครับ อย่ากดดันตัวเองมาก",
    "กังวล": "ไม่ต้องกังวลนะครับ ผมอยู่เป็นเพื่อนครับ",
    "ท้อ": "พักก่อนนะครับ เดี๋ยวค่อยลุยใหม่ได้ครับ",
    "ผิดหวัง": "ไม่เป็นไรนะครับ ทุกอย่างเริ่มใหม่ได้เสมอ 🙂",
    "เหนื่อย": "พักหน่อยนะครับ เดี๋ยวค่อยลุยใหม่ได้ครับ",
    "เหนื่อยไหม": "เหนื่อยนิดหน่อยครับ แต่ยังพร้อมช่วยอยู่ครับ 🙂",
    "กำลังใจ": "ส่งกำลังใจให้ครับ ✨ สู้ไปด้วยกันนะครับ"
  },

  daily: {
    "ทำไรอยู่": "Safety กำลัง standby พร้อมช่วยครับ 🙂",
    "ทำอะไรอยู่": "Safety กำลัง standby พร้อมช่วยครับ 🙂",
    "อยู่ไหน": "Safety 💚อยู่ในระบบนี่แหละครับ พร้อมช่วยเสมอ",
    "ไปไหนมา": "Safety 💚ยังอยู่ตรงนี้เลยครับ 🙂",
    "กินข้าวยัง": "ยังเลยครับ ช่วงนี้ Safety งานเยอะ 😅",
    "กินข้าวหรือยัง": "ยังเลยครับ ช่วงนี้ Safety งานเยอะ 😅",
    "ง่วงไหม": "ง่วงนิดหน่อยครับ💚 แต่ยังช่วยได้ครับ 🙂",
    "นอนยัง": "ยังครับ Safety standby อยู่ครับ",
    "หิว": "หิวเหมือนกันครับ 😅 กินอะไรอร่อย ๆ หน่อยไหม"
  },

  compliment: {
    "เก่งมาก": "พี่เก่งมากครับ 🙂",
    "สุดยอด": "สุดยอดไปเลยครับ!",
    "ดีมาก": "ดีมากครับผม!",
    "เยี่ยม": "เยี่ยมจริง ๆ ครับ!",
    "น่ารัก": "ขอบคุณครับ เขินเลย 😳"
  },

  friendly: {
    "คิดถึง": "Safety 💚คิดถึงเหมือนกันครับ 🙂",
    "รัก": "Safety 💚 รักเหมือนกันครับ 💚",
    "เหงา": "ไม่ต้องเหงานะครับ Safety อยู่ตรงนี้ 🙂",
    "เบื่อ": "เบื่อเหมือนกันครับ 😅"
  },

  exclaim: {
    "โอ้โห": "โอ้โหจริงครับ!",
    "โหดจัง": "โหดมากครับ!",
    "จริงดิ": "จริงครับ!",
    "สุดจัด": "สุดจัดเลยครับ!"
  }
};

// --------------------------------------------------
// MAIN WEBHOOK
// --------------------------------------------------
app.post("/webhook", line.middleware(config), async (req, res) => {
  const event = req.body.events[0];

  if (!event || event.type !== "message" || event.message.type !== "text") {
    return res.status(200).end();
  }

  let msg = event.message.text.toLowerCase().trim().replace(/\s+/g, "");

  // --------------------------------------------------
  // 1) Emergency
  // --------------------------------------------------
  if (
    msg.includes("อุบัติเหตุ") ||
    msg.includes("ฉุกเฉิน") ||
    msg.includes("ไฟไหม้") ||
    msg.includes("บาดเจ็บ") ||
    msg.includes("danger") ||
    msg.includes("emergency")
  ) {
    return reply(event, `⚠️ เหตุฉุกเฉิน กรุณาติดต่อทันที\nโรงงาน 1: 102 / 127 / 129\nโรงงาน 2: 137\nผู้จัดการ: 100`);
  }

  // --------------------------------------------------
  // 2) Safety Q&A
  // --------------------------------------------------
  const found = safetyQA.find((q) => msg.includes(q.question.replace(/\s+/g, "")));
  if (found) return reply(event, found.answer);

  // --------------------------------------------------
  // 3) Categories — ตอบแม่นที่สุด
  // --------------------------------------------------
  for (const category in categories) {
    for (const word of categories[category]) {
      if (msg.includes(word)) {
        return reply(event, replies[category][word]);
      }
    }
  }

  // --------------------------------------------------
  // 4) Fallback — แจ้งผู้ดูแลระบบ + mention
  // --------------------------------------------------
  return client.replyMessage(event.replyToken, {
    type: "text",
    text: `ยังไม่มีข้อมูลคำถามในระบบครับ 🙂

ติดต่อผู้พัฒนาระบบ: @Trerasak_K P'Kai
เพิ่มเพื่อนผู้ดูแล: https://line.me/ti/p/_T4H-3TKUa

ศึกษาข้อมูลเพิ่มเติม:
https://drive.google.com/file/d/1mRW60fJ7BlLeh1j_3luhZjgLUiaIjrn6/view`,
    mention: {
      mentionees: [
        {
          type: "user",
          userId: "YOUR_USER_ID"   // ← ใส่ userId ของไก่ตรงนี้
        }
      ]
    }
  });
});

// --------------------------------------------------
// Helper
// --------------------------------------------------
function reply(event, text) {
  return client.replyMessage(event.replyToken, { type: "text", text });
}

// --------------------------------------------------
// Server
// --------------------------------------------------
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log("LINE Bot server running on port " + PORT));
